...
class dbuser {
    ...
    public function dangKy($username, $password, $email, $display_name = '') {
        ...
        // Nếu tồn tại username giống nhau → return false
        ...
        // Nếu tồn tại email giống nhau → return false
        if ($result->num_rows > 0) {
    ...
    public function doiMatKhau($user_id, $mat_khau_cu, $mat_khau_moi) {
        ...
        $stmt = $this->conn->prepare($sql);
        $stmt->bind_param("si", $mat_khau_moi_md5, $user_id);
    ...
    public function login($username, $password) {
        ...
        while ($row = $result->fetch_assoc()) {
            // Nếu tìm thấy username và password mã hóa khớp (không check role, ai cũng đăng nhập được)
